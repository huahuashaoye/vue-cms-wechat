		...
		<el-card class="box-card">
			...
			<el-form ref="form" :model="form" label-width="100px" :rules="rules">
				...
				<el-form-item label="商品轮播图" prop="slider">
					...
					<el-upload action="/api/upload/slider"  :file-list="photo" list-type="picture-card"
...
<script>
	...
	export default {
		...
		async created() {
			let id = this.id;
			let res = await Commodity.goods({ id });
			this.form = res.data;
			...
			let { data, msg, status } = await Commodity.all({ pId: 1 });
		...
		methods: {
			async loadSubcate(pId) {
				let { status, data } = await Commodity.all({ pId });
				return data;
			},
			onSubmit() {
				console.log('submit!');
			...
			submitForm(formName) {
				this.$refs[formName].validate(async (valid) => {
					if (valid) {
						let { status, msg } = await Commodity.editGoods({ ...this.form });
						if (status) {
